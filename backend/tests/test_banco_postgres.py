...
class TestRegistrarUsuario:
    def test_insert_executado(self, conn):
        mock_conn, mock_cur = conn
        bp.registrar_usuario(mock_conn, 'Almodenga', 'senha123')
        mock_cur.execute.assert_called_once()
        sql, params = mock_cur.execute.call_args.args
        assert 'INSERT' in sql
        assert params[0] == 'Almodenga'
    ...
    def test_retorna_true_em_sucesso(self, conn):
        mock_conn, mock_cur = conn
        resultado = bp.registrar_usuario(mock_conn, 'Almodenga', 'senha123')
...
class TestAutenticar:
    def test_retorna_true_se_encontrou(self, conn):
        ...
        assert bp.autenticar(mock_conn, 'Almodenga', 'senha123') is True

    def test_retorna_false_se_nao_encontrou(self, conn):
        mock_conn, mock_cur = conn
        mock_cur.fetchone.return_value = None
        assert bp.autenticar(mock_conn, 'Almodenga', 'senhaerrada') is False

    def test_usa_hash_na_query(self, conn):
        mock_conn, mock_cur = conn
        mock_cur.fetchone.return_value = None
        bp.autenticar(mock_conn, 'Almodenga', 'senha123')
...
class TestUsuarioExiste:
    def test_existe(self, conn):
        mock_conn, mock_cur = conn
        mock_cur.fetchone.return_value = (1,)
        assert bp.usuario_existe(mock_conn, 'Almodenga') is True

    def test_nao_existe(self, conn):
        mock_conn, mock_cur = conn
        mock_cur.fetchone.return_value = None
        assert bp.usuario_existe(mock_conn, 'Almodenga') is False
...
class TestObterPerfil:
    def test_retorna_perfil_completo(self, conn):
        mock_conn, mock_cur = conn
        mock_cur.fetchone.return_value = ('Almodenga', 6, 10)
        perfil = bp.obter_perfil(mock_conn, 'Almodenga')
        assert perfil['nome'] == 'Almodenga'
    ...
    def test_derrotas_calculadas(self, conn):
        mock_conn, mock_cur = conn
        mock_cur.fetchone.return_value = ('Baguga', 3, 8)
        perfil = bp.obter_perfil(mock_conn, 'Baguga')
    ...
    def test_derrotas_zero(self, conn):
        mock_conn, mock_cur = conn
        mock_cur.fetchone.return_value = ('Almodenga', 5, 5)
        perfil = bp.obter_perfil(mock_conn, 'Almodenga')
...
class TestObterRanking:
    def test_retorna_lista_ordenada(self, conn):
        mock_conn, mock_cur = conn
        mock_cur.fetchall.return_value = [('Almodenga', 10), ('Baguga', 7), ('CharlesHenrique', 3)]
        ranking = bp.obter_ranking(mock_conn)
        assert len(ranking) == 3
        assert ranking[0] == {'nome': 'Almodenga', 'vitorias': 10}
        assert ranking[1] == {'nome': 'Baguga', 'vitorias': 7}
...
class TestRegistrarResultado:
    def test_vitoria_incrementa_vitorias_e_partidas(self, conn):
        mock_conn, mock_cur = conn
        bp.registrar_vitoria(mock_conn, 'Almodenga')
        sql, params = mock_cur.execute.call_args.args
    ...
    def test_derrota_incrementa_so_partidas(self, conn):
        mock_conn, mock_cur = conn
        bp.registrar_derrota(mock_conn, 'Baguga')
        sql, params = mock_cur.execute.call_args.args
        assert 'partidas = partidas + 1' in sql
        assert 'vitorias' not in sql
        assert params[0] == 'Baguga'
        mock_conn.commit.assert_called_once()
...
class TestEncerrarPartida:
    def test_update_executado(self, conn):
        mock_conn, mock_cur = conn
        bp.encerrar_partida(mock_conn, 7, ['Almodenga', 'CharlesHenrique'], ['Baguga', 'Dani'], 'completa')
        sql, params = mock_cur.execute.call_args.args
    ...
    def test_vencedores_em_string(self, conn):
        ...
        bp.encerrar_partida(mock_conn, 7, ['Almodenga', 'CharlesHenrique'], ['Baguga', 'Dani'], 'completa')
        _, params = mock_cur.execute.call_args.args
        assert params[0] == 'Almodenga,CharlesHenrique'
        assert params[1] == 'Baguga,Dani'

    def test_status_wo(self, conn):
        mock_conn, mock_cur = conn
        bp.encerrar_partida(mock_conn, 7, ['Almodenga'], ['Baguga'], 'wo')
        _, params = mock_cur.execute.call_args.args
        assert params[2] == 'wo'

    def test_commit_apos_update(self, conn):
        mock_conn, mock_cur = conn
        bp.encerrar_partida(mock_conn, 7, ['Almodenga'], ['Baguga'], 'completa')
        mock_conn.commit.assert_called_once()
